...
Процедура ОповещениеПрочитатьФайл(ПомещенныеФайлы, ЭтотОбъект)Экспорт
	...
	Если ТипЗнч(ПомещенныеФайлы) = Тип("Массив") Тогда
		...
		ДанныеСтруктура._Автор		= ?(ТипЗнч(ДанныеСтруктура._Автор) 		= Тип("ОбъектXDTO"), "", ДанныеСтруктура._Автор);
		ДанныеСтруктура.Подсказка	= ?(ТипЗнч(ДанныеСтруктура.Подсказка) 	= Тип("ОбъектXDTO"), "", ДанныеСтруктура.Подсказка);
		ДанныеСтруктура.Коммент		= ?(ТипЗнч(ДанныеСтруктура.Коммент) 	= Тип("ОбъектXDTO"), "", ДанныеСтруктура.Коммент);
		...
		Для каждого СтрДанных Из ДанныеXDTO.ТЧТело.Row Цикл
			...
			ОписаниеСтрокиТЧ.СтрокаДанных 	= ?(ТипЗнч(СтрДанных.СтрокаДанных) 	= Тип("ОбъектXDTO"), "", СтрДанных.СтрокаДанных);
			ОписаниеСтрокиТЧ.Подсказка 		= ?(ТипЗнч(СтрДанных.Подсказка) 	= Тип("ОбъектXDTO"), "", СтрДанных.Подсказка);
		...
		Если ДанныеXDTO.СодержитКартинку Тогда
			...
			КартинкаСтруктура.Коммент = ?(ТипЗнч(ДанныеXDTO._Визуализация.Коммент) = Тип("ОбъектXDTO"), "", ДанныеXDTO._Визуализация.Коммент);
...
Процедура СоздатьЭлементыНаОсновеДанныхФайлаXML(ДанныеСтруктура)
	...
	Если ЗначениеЗаполнено(ДанныеСтруктура._Автор) Тогда
	
		АвторСсылка = Справочники.Авторы.НайтиПоНаименованию(ДанныеСтруктура._Автор).Ссылка;
		ПроизведениеСсылка.Автор = АвторСсылка;
		
		Если НЕ ЗначениеЗаполнено(АвторСсылка) Тогда
			
			ОшибкаПолученияАвтора = НСтр(
			"ru = ""Ошибка Автор не найден по наименованию в справочнике ""Авторы""!"";
			|en = ""Error Author not found by name in ""Authors"" catalog!""
			|");
			ПроизведениеСсылка.Коммент = ОшибкаПолученияАвтора + Символы.ПС + ПроизведениеСсылка.Коммент;
